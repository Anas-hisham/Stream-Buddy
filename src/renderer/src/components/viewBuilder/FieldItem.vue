<template>
  <div :style="{
    ...
    <div class="flex items-center gap-3 w-full">
      <i v-if="newView.layoutStyle === 'grid'" class="pi pi-circle-fill field-handle cursor-grab text-sm flex-shrink-0"
        :class="displayMode === 'dark' ? 'text-gray-400' : 'text-gray-500'"></i>

      <div class="flex-grow">
        <div class="relative">
          <input type="text" v-model.lazy="field.name" @change="validateFieldName(sectionIndex, lineIndex, fieldIndex)"
            ...
            :class="[
              displayMode === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-300',
      ...
      <div :class="[fieldErrors[`${sectionIndex}-${lineIndex}-${fieldIndex}`] ? 'pb-10' : '']"></div>
      <div class="flex-shrink-0" style="max-width: 80px">
        <div class="relative w-full">
          ...
          <i
            class="pi pi-chevron-down absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none opacity-70"></i>
        </div>
      </div>

      <button @click="removeField(sectionIndex, lineIndex, fieldIndex)"
    ...
    <p v-if="fieldErrors[`${sectionIndex}-${lineIndex}-${fieldIndex}`]"
