<template>
  <div
    :class="['rounded-lg shadow-md p-6 mb-6', displayMode === 'dark' ? 'bg-gray-700' : 'bg-white']"
  >
    <div class="text-center mb-10 p-6 rounded-lg">
      <p
        :class="['text-lg font-medium', displayMode === 'dark' ? 'text-gray-200' : 'text-gray-700']"
      >
      ...
      <button
        ...
        class="mt-4 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg shadow-sm transition-colors"
      >
        Check for Update
      </button>
      ...
      <div v-if="autoUpdateStore.showUpdateUI" class="mt-6 space-y-4">
        <div v-if="autoUpdateStore.updateAvailable && autoUpdateStore.showDownloadButton">
          <button
            @click="startDownload"
            class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg shadow-sm transition-colors"
          >
            Download Update
          </button>
        </div>

        <p class="text-sm" :class="displayMode === 'dark' ? 'text-blue-300' : 'text-blue-600'">
