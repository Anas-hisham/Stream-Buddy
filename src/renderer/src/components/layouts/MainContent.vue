<template>
  <main
    :class="[
      ...
      appStore.settings.displayMode === 'dark' ? 'bg-[#2a3444] text-white' : 'bg-white text-black'
    ]"
  >
    <component :is="currentViewComponent" v-if="currentViewComponent" v-bind="currentViewProps" />
    <Spinner v-else />
  </main>
</template>
...
const currentViewComponent = computed(() => {
  ...
  const isCustomView = appStore.allViews.some((view) => view.path === props.path && view.config)
