  ...
  <div :style="{
    ...
    <div class="grid gap-1 relative">
      <label class="text-sm font-semibold flex items-baseline gap-1">
        <i v-if="layoutStyle === 'grid'" class="pi pi-circle-fill field-handle cursor-grab text-xs"
          :class="displayMode === 'dark' ? 'text-gray-400' : 'text-gray-500'"></i>
        {{ field.name }}
      </label>

      <template v-if="field.type === 'text' || field.type === 'integer'">
        <div class="mt-1 p-3 max-h-12 border relative rounded-md"
          :class="displayMode === 'dark' ? 'border-white' : 'border-black'">
          <i class="pi pi-pencil absolute left-2.5 top-1/2 -translate-y-1/2"
            :class="displayMode === 'dark' ? 'text-gray-400' : 'text-gray-500'"></i>
          <input :type="field.type === 'integer' ? 'number' : 'text'" :value="field.value" @input="
            (e) =>
              updateFieldValue(
                sectionIndex,
                lineIndex,
                fieldIndex,
                field.type === 'integer' ? parseInt(e.target.value) || 0 : e.target.value
              )
          " class="outline-none pl-5 w-full bg-transparent" :class="displayMode === 'dark'
      ...
      <template v-else-if="field.type === 'image'">
        <div class="flex justify-between border px-4 h-12 items-center gap-3 rounded-md mt-1"
          :class="displayMode === 'dark' ? 'border-white' : 'border-black'">
          <span class="text-sm opacity-80">{{ field.name }}</span>
          <div v-if="field.value" class="flex items-center gap-2">
            <img :src="field.value" alt="Uploaded image" class="w-10 h-10 object-cover cursor-pointer rounded"
              @click="openImageDialog(sectionIndex, lineIndex, fieldIndex)" />
            <button type="button" class="text-sm text-red-600 font-medium hover:underline"
              @click="removeImage(sectionIndex, lineIndex, fieldIndex)">
              Delete
            </button>
          </div>
          <button v-else class="text-sm font-medium"
